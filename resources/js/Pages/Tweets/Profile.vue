        ...
        <template #header>
            <div class="flex items-center justify-between">
                ...
                <div v-if="profileUser.id !== $page.user.id">
                    <inertia-link v-if="!profilUser.is_followed" as="button" method="POST" :href="`/follows/${profileUser.id}`" class="bg-white text-blue-500 cursor-pointer px-5 py-2 hover:text-white border border-blue-500 leading-tight hover:bg-blue-500 rounded-full font-extrabold transition-all duration-300" preserve-scroll>Suivre</inertia-link>
                </div>
            </div>
        </template>
